<template>
    <app-layout>
        <!-- show alert if no backups yet -->
        <div class="flex items-center justify-center w-full">
            <div v-if="!hasBackup" class="m-2">
                <div
                    class="
                        flex
                        max-w-sm
        ...
        <!-- latest backup stats -->
        <Stats :props="$page.props" />
    </app-layout>
</template>

<script lang="ts">
import { Options, Vue } from "vue-class-component";
import Stats from "../components/settings/Stats.vue";

@Options({ components: { Stats } })
export default class Settings extends Vue {
    hasBackup: boolean = true;

    mounted() {
        this.hasBackup = this.$page.props["has_backup"] ? false : true;
    }
}
</script>
